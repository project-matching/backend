...
class BookMarkControllerTest {
    ...
    @Test
    public void 즐겨찾기_등록_테스트() throws Exception {
        // given
        ...
        mvc.perform(post("/v1/bookmark/" + saveProject1.getNo()).contentType(MediaType.APPLICATION_JSON)
    ...
    @Test
    public void 즐겨찾기_프로젝트_조회_테스트() throws Exception {
        // given
        LocalDateTime createDate = LocalDateTime.now();
        LocalDate startDate = LocalDate.of(2022, 06, 24);
        LocalDate endDate = LocalDate.of(2022, 06, 28);
        ...
        mvc.perform(get("/v1/bookmark?page=0&size=5&sortBy=createDate,desc")
                        .contentType(MediaType.APPLICATION_JSON)
                        .header("Authorization", "Bearer " + token))
                .andDo(print())
                .andExpect(header().string("Content-type", "application/json"))
                .andExpect(jsonPath("$.data.content.length()").value(1))
                .andExpect(jsonPath("$.data.content[0].name").value(saveProject1.getName()))
                .andExpect(jsonPath("$.data.content[0].maxPeople").value(saveProject1.getMaxPeople()))
                .andExpect(jsonPath("$.data.content[0].currentPeople").value(saveProject1.getCurrentPeople()))
                .andExpect(jsonPath("$.data.content[0].viewCount").value(saveProject1.getViewCount()))
                .andExpect(jsonPath("$.data.content[0].register").value(saveProject1.getCreateUserName()))
                ...
                .andExpect(jsonPath("$.data.content[0].projectSimplePositionDtoList[1].projectNo").value(saveProjectPosition2.getProject().getNo()))
                .andExpect(jsonPath("$.data.content[0].projectSimplePositionDtoList[1].positionNo").value(saveProjectPosition2.getPosition().getNo()))
    ...
    @Test
    public void 즐겨찾기_삭제_테스트() throws Exception {
        // given
        LocalDateTime createDate = LocalDateTime.now();
        LocalDate startDate = LocalDate.of(2022, 06, 24);
        LocalDate endDate = LocalDate.of(2022, 06, 28);
        ...
        mvc.perform(delete("/v1/bookmark/" + saveProject1.getNo()).contentType(MediaType.APPLICATION_JSON)
                        .header("Authorization", "Bearer " + token))
                .andDo(print())
                .andExpect(header().string("Content-type", "application/json"))
                .andExpect(jsonPath("$.data").value(true))
